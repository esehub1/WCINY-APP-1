buildscript {
    ext {
        ...
        // Ensure we have a usable gradle tools version (fallback to a sane default)
        if (ANDROID_GRADLE_TOOLS_VERSION == null || ANDROID_GRADLE_TOOLS_VERSION.toString().trim().length() == 0) {
            ANDROID_GRADLE_TOOLS_VERSION = '4.1.0'
        }
        // Guard against null or malformed values before parsing the numeric gradle version
        try {
            if (ANDROID_GRADLE_TOOLS_VERSION != null && ANDROID_GRADLE_TOOLS_VERSION.lastIndexOf('.') != -1) {
                ext.gradleNumberVersion = ANDROID_GRADLE_TOOLS_VERSION.substring(0, ANDROID_GRADLE_TOOLS_VERSION.lastIndexOf('.')).toFloat()
            } else {
                // fallback to a reasonable default
                ext.gradleNumberVersion = 4.1f
            }
        } catch(Exception __e) {
            ext.gradleNumberVersion = 4.1f
        }
    ...
    dependencies {
        ...
        ANDROID_GRADLE_TOOLS_VERSION = rootProject.hasProperty('ANDROID_GRADLE_TOOLS_VERSION') ? rootProject.ext.ANDROID_GRADLE_TOOLS_VERSION : ANDROID_GRADLE_TOOLS_VERSION
        // Ensure versions are defined to avoid creating a null classpath
        if (ANDROID_GOOGLE_SERVICES_VERSION == null || ANDROID_GOOGLE_SERVICES_VERSION.toString().trim().length() == 0) {
...
// Only apply Google Services plugin when building the app module itself and when app's google-services.json exists.
try {
    def appGs = rootProject.file('app/google-services.json')
    if (appGs.exists() && (project.name == 'app' || project.path == ':app')) {
        if (!project.plugins.hasPlugin('com.google.gms.google-services')) {
            apply plugin: com.google.gms.googleservices.GoogleServicesPlugin
        }
    } else {
        print("FCMPlugin: google-services.json not found in app module or current project is not 'app'; skipping Google Services plugin apply for project: ${project.name}.")
    }
} catch(Exception e) {
    print("FCMPlugin: Unable to check for google-services.json or project name; skipping apply to avoid build failure.")
}
